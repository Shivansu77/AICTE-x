...
import { BookOpen, Home, Settings, User, Bell, LogOut } from 'lucide-react';
...
const Layout = ({ children }) => {
  ...
  const role = "Faculty"; // TODO: Get from auth context
      ...
      <aside className="w-72 flex flex-col">
        ...
        <div className="px-4 mt-auto">
             <div className="bg-accent-yellow/20 p-6 rounded-3xl flex flex-col items-center text-center gap-3">
                <div className="w-16 h-16 bg-accent-yellow rounded-full flex items-center justify-center text-2xl font-bold text-white shadow-sm">
                    {role[0]}
                </div>
                <div>
                    <h3 className="font-bold text-lg">Dr. Sharma</h3>
                    <p className="text-sm text-secondary opacity-80">{role}</p>
                </div>
                <button className="flex items-center gap-2 text-sm font-bold text-secondary hover:text-primary mt-2">
