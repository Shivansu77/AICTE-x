...
const ApprovalDashboard = () => {
    ...
    if (loading) return (
        <div className="flex items-center justify-center h-screen bg-cream dark:bg-primary">
    ...
    return (
        <div className="min-h-[calc(100vh-2rem)] bg-cream dark:bg-primary rounded-[2.5rem] border border-white dark:border-border-color shadow-xl p-6">
            <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 mb-6">
                <div>
                    <h1 className="text-3xl font-black text-primary">AI Approvals</h1>
                    <p className="text-sm text-secondary">Select a request to open the AI review workspace.</p>
                </div>
                <div className="flex flex-wrap items-center gap-3">
                    <div className="px-4 py-2 rounded-full bg-white dark:bg-card border border-gray-100 dark:border-border-color text-sm font-bold text-primary">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[360px_1fr] gap-6">
                <aside className="space-y-4">
                    ...
                    <div className="bg-white dark:bg-card rounded-2xl border border-gray-100 dark:border-border-color p-4 shadow-sm">
                        <div className="text-xs font-black uppercase tracking-wider text-secondary">Search Requests</div>
                        <input
                            value={query}
                            onChange={(e) => setQuery(e.target.value)}
                            placeholder="Search by course, faculty, or keyword"
                            className="mt-2 w-full rounded-xl border border-gray-200 dark:border-border-color px-3 py-2 text-sm bg-white dark:bg-card text-primary focus:outline-none focus:ring-2 focus:ring-blue-200"
                        />
                    </div>
                </aside>

                <section className="bg-white dark:bg-card rounded-2xl border border-gray-100 dark:border-border-color p-5 shadow-sm">
                    <div className="flex items-center justify-between mb-4">
                        <h2 className="text-lg font-black text-primary">Approval Queue</h2>
                        <span className="text-xs text-secondary">Click a request to review</span>
                    </div>
